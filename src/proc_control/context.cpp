...
void awake(EMProc& emproc, struct timespec ts, int tun_fd) {
    ...
    while (true) {
        ...
        if ((ssize = read(tun_fd, buf, sizeof(buf))) < 0) {
            ...
            emproc.out_packets.add_packet(buf, ssize, elapsed_time);
        }

        /* If anything should be sent to this process in this loop, send it */
        while (!emproc.in_packets.packets.empty() && 
               is_greater(elapsed_time, emproc.in_packets.get_first_ts())) {
            /* First packet from to_receive can be sent */

            Packet packet = emproc.in_packets.pop_first_packet();
            // TODO sending packet.
        }
    }
